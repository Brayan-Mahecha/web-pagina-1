        ...
        <div class="d-flex justify-content-center h-100">
            <div class="card" style="height: 40rem">
                <div class="card-header">
                    <br>
                    <h3 align=center>SELECCIONE EMPLEADO A MODIFICAR</h3>
                ...
                <div class="card-body">
                    ...
                    $registros = mysqli_query($conexion, "select EmpCedula,EmpNombre from empleados") or
                        die("Problemas en el select:" . mysqli_error($conexion));
                    $contador = 0;
                    echo '<div class="container bg-white"><br>';
                    echo "<div class='btn-group-vertical btn-group-sm d-grid gap-3'>";
                    while ($reg = mysqli_fetch_array($registros)) {
                        $dato = $reg['EmpCedula'];
                        if ($contador % 2 == 0) {
                            echo "<form action=updateemp.php method='post'>";
                            echo "<button name='cedula' type='submit' class='btn btn-light btn-lg border' value=$dato>" . "Identificación:\t" . $reg['EmpCedula'] . "\tNombre:\t" . $reg['EmpNombre'] . "</button>";
                            echo "</form>";
                        } else {
                            echo "<form action=updateemp.php method='post'>";
                            echo "<button name='cedula' type='submit' class='btn btn-secondary btn-lg'  value=$dato>" . "Identificación:\t" . $reg['EmpCedula'] . "\tNombre:\t" . $reg['EmpNombre'] . "</button>";
                            echo "</form>";
                        }
                        $contador++;
                    }
