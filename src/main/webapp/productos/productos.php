    ...
    <div class="d-flex justify-content-center h-100">
      <div class="card" style="height: 40rem">
        <div class="card-header">
          <br>
          <h3 align=center>SELECCIONE UN PRODUCTO A MODIFICAR</h3>
        ...
        <div class="card-body">
          ...
          $registros = mysqli_query($conexion, "select ProNombre,ProPrecio, ProdID from productos") or
          ...
          while ($reg = mysqli_fetch_array($registros)) {
            $dato = $reg['ProdID'];
            if ($contador % 2 == 0) {
              echo "<form action=updateprod.php method='post'>";
              echo "<button name='nombre' type='submit' class='btn btn-light btn-lg border' value=$dato>" . "Nombre:\t" . $reg['ProNombre'] . "\tPrecio:\t" . $reg['ProPrecio'] . "</button>";
              echo "</form>";
            } else {
              echo "<form action=updateprod.php method='post'>";
              echo "<button name='nombre' type='submit' class='btn btn-secondary btn-lg' value=$dato>" . "Nombre:\t" . $reg['ProNombre'] . "\tPrecio:\t" . $reg['ProPrecio'] . "</button>";
              echo "</form>";
            }
            $contador++;
          }
